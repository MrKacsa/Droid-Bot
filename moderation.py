...
class Moderation(commands.Cog, name="moderation"):
    ...
    async def kick(
        ...
        if member.guild_permissions.administrator:
            ...
            try:
                embed = discord.Embed(
                    description=f"**<:gtFireWand:1057970317521334354> {member}** was kicked by **{context.author}**!",
                    color=0x9C84EF,
                )
    ...
    async def hackban(
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"**<:gtCurseWand:1057970162692796507> {user}** (ID: {user_id}) was Cursly banned by **{context.author}**!",
                color=0x9C84EF,
            )
            embed.add_field(name="Reason:", value=reason)
            await context.send(embed=embed)
