...
class Owner(commands.Cog, name="owner"):
    ...
    @commands.hybrid_command(
      name="show_servers",
      description="Shows the list of servers the bot is in along with invite links.",
  )
    @checks.is_owner()
    async def show_servers(self, context: Context) -> None:
      """
      Shows the list of servers the bot is in with invite links.

      :param context: The hybrid command context.
      """
      guilds = self.bot.guilds
      embed = discord.Embed(title="Servers I'm in", color=0x9C84EF)

      for guild in guilds:
          invite_link = await guild.text_channels[0].create_invite(max_age=86400, max_uses=1)
          embed.add_field(name=guild.name, value=f"ID: {guild.id}\n[Invite Link]({invite_link})", inline=False)
    ...
    async def sync(self, context: Context, scope: str) -> None:
        ...
        if scope == "global":
            ...
            embed = discord.Embed(
                description="<:Agree:1058864116288192592> Slash commands have been globally synchronized.",
                color=0x9C84EF,
            )
            await context.send(embed=embed)
            return
            ...
            embed = discord.Embed(
                description="<:Agree:1058864116288192592> Slash commands have been synchronized in this guild.",
                color=0x9C84EF,
            )
            await context.send(embed=embed)
            return
    ...
    async def unsync(self, context: Context, scope: str) -> None:
        ...
        if scope == "global":
            ...
            await context.bot.tree.sync()
            embed = discord.Embed(
                description=" <:Agree:1058864116288192592> Slash commands have been globally unsynchronized.",
                color=0x9C84EF,
            )
            await context.send(embed=embed)
            return
            ...
            await context.bot.tree.sync(guild=context.guild)
            embed = discord.Embed(
                description="<:Agree:1058864116288192592> Slash commands have been unsynchronized in this guild.",
                color=0x9C84EF,
            )
            await context.send(embed=embed)
            return
